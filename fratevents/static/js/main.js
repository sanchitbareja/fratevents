	...
	$.get('get/events/',function(data){
		...
		for(var party_date in data['events']){
			...
			for(var event in data['events'][party_date][1]){
				...
				newa.appendChild(document.createTextNode("Hosted by "+data['events'][party_date][1][event]['host']+"<br />@"+data['events'][party_date][1][event]['host']+", "+formatAMPM(new Date(data['events'][party_date][1][event]['startTime']))));
				var newli = document.createElement("li");
